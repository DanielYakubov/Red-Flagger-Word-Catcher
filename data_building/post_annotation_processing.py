...
def filter_overlaps_and_sort(word_list: list[str]) -> list[str]:
    ...
    1) All the single gram items are extracted into a list,
    iff they are unique to the list. Multi-words are stored.
    2) The multi-word container is iterated and added to the unique words list
    iff the multi-words do not contain
        existing single words.

    This algorithm also pseudo-sorts the list. First,
    all the unigrams are listed, then the n>1 gram items.
    """
    # NOT using a set here,
    # want to be careful about determinism & insertion order.
    ...
    [
        unique_words.append(x[0])
        for x in split_word_list
        if len(x) == 1 and x[0] not in unique_words
    ]
    [
        multi_words.append(x)
        for x in split_word_list
